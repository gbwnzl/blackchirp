...
# this file is taken from the NVidia PicoEVB example for Jetson
...
#		crc=00000000
        # if we did not find a function we abort and remove the file
        rm "${symvers_fn}"
        echo "Symbol not found => try nvidia-p2p.ko"
        exit 0
	fi
